...
void  print_help(){
   char *msg="Usage: %s [OPTION] \n \
	  ...
	  -p path to the file  \n \
          ...
          Example:  aprun -n 1 ./read  -f /scratch3/scratchdirs/dbin/testf.h5   -d /testg/testd -q :,:,1:1000 \n ";
...
int main(int argc, char **argv){
  ...
  unsigned short *data;
  int             rank, mpi_size, mpi_rank, c, i;
  ...
  //Register SDS VOL plugin to use
  under_dapl = H5Pcreate(H5P_FILE_ACCESS);
  H5Pset_fapl_mpio(under_dapl, comm, info);
  vol_id  = H5VLregister(&H5VL_sds_external_g);
  native_plugin_id = H5VLget_plugin_id("native");
  assert(native_plugin_id > 0);
  dacc_tpl = H5Pcreate(H5P_FILE_ACCESS);
  H5Pset_vol(dacc_tpl, vol_id, &under_dapl);


  // Open file and Dataset  
  file_id          = H5Fopen(filename, H5F_ACC_RDONLY, dacc_tpl);
