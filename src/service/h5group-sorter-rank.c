...
void package(char *p_data, int file_index, int my_data_size, char *my_data, int row_size);
void unpackage(char *p_data, int file_index, int my_data_size, char *my_data, int row_size);
...
unsigned long long getULLValue(int index, char *row_data);
...
int phase1(int mpi_rank, int mpi_size, char *data, int my_data_size, char *sample_of_rank0, int row_size);

//Phase 2 of the parallel sampling sorting
int phase2(int mpi_rank, int mpi_size, char *data, int my_data_size, char *pivots, int rest_size, int row_size);

// Master does slave's job, and also gather and sort pivots
int master(int mpi_rank, int mpi_size, char *data, int my_data_size, int rest_size, int row_size);

/*Do sort and sample*/
int slave(int mpi_rank, int mpi_size, char *data, int my_data_size, int rest_size, int row_size);

//Sort the data based on the type
int qsort_type(void *data, size_t my_data_size, size_t row_size);
...
void rank_pivot(char *pivots, char *data,  int my_data_size, int dest_pivot, int *rank_less, int *rank_equal, int row_size, int mpi_size, double p_value_head);
...
void  print_help(){
  char *msg="Usage: %s [OPTION] \n\
          ...
          -e only sort the key  \n \
          -v verbose  \n \
          example: ./h5group-sorter -f testf.h5p  -g /testg  -o testg-sorted.h5p -a testf.attribute -i 0 \n";
...
int main(int argc, char **argv){
  ...
  key_index = 1;  
  int c;
  static const char *options="f:o:a:g:k:hsveml:r";
  extern char *optarg;
  ...
  //Attach the rank to the end
  //unsigned long long
  if(rank_combine == 1){
    strncpy(dname_array[dataset_num].dataset_name, "rank", NAME_MAX);
    dname_array[dataset_num].type_id   =  H5T_STD_I64LE; //or H5T_STD_U64LE
    dname_array[dataset_num].type_size =  sizeof(unsigned long long);
    if( max_type_size < dname_array[dataset_num].type_size){
      max_type_size = dname_array[dataset_num].type_size;
    }
    dname_array[dataset_num].did       =  -1;    
  ...
  if(mpi_rank == 0 || mpi_rank == (mpi_size -1)){
    printf(" Data patition (based on key) My rank: %d, file size: %lu, my_data_size: %lu, my_offset: %lu , row_size %d, char size %d , dataset_num = %d row_count = %d, max type size %d, sizeof(MPI_CHAR)=%d, sizeof(unsigned long long int) = %d \n ", mpi_rank, (unsigned long)dims_out[0],(unsigned long)my_data_size, (unsigned long)my_offset, row_size, sizeof(char), dataset_num, row_count, max_type_size, sizeof(MPI_CHAR), sizeof(unsigned long long int));
  }
  ...
  unsigned long long *rank_buf;
  MPI_Barrier(MPI_COMM_WORLD);
  t20 = MPI_Wtime();
  unsigned long long iiii;
  for (i = 0 ; i < dataset_num; i++){
    //Read one dataset
    if(rank_combine == 1 && (i == (dataset_num -1))){
      free(temp_data);
      temp_data = NULL;
      
      rank_buf = malloc(my_data_size * sizeof(unsigned long long));
      if(rank_buf == NULL){
        printf("Rank buf allocation failed !\n");
        exit(-1);
      }
      for(iiii = 0; iiii < my_data_size; iiii++){
        rank_buf[iiii] =   my_offset + iiii;
      }
      package(package_data, i, row_count, (char *)rank_buf, row_size);
      free(rank_buf);
    }else{
    ...
    if(mpi_rank == 0)
      printf("%d,  %s , type id (%d), type size (%d) key %f, rank %f  \n ", i, dname_array[i].dataset_name, dname_array[i].type_id, dname_array[i].type_size,  get_value_double(key_index, package_data), get_value_double(key_index+1, (package_data+row_size*2)));
  }
...
int CompareULLKeyRank(const void *a, const void *b){
  unsigned long long v1, v2;
  v1 = getULLValue(key_index+1, (char *)a);
  v2 = getULLValue(key_index+1, (char *)b);
...
int CompareInt32KeyRank(const void *a, const void *b){
  int v1, v2;
  v1 = getInt32Value(key_index+1, (char *)a);
  v2 = getInt32Value(key_index+1, (char *)b);
...
int phase1(int mpi_rank, int mpi_size, char *data, int my_data_size, char *sample_of_rank0, int row_size){
  ...
  if (mpi_rank != 0){
    ...
    for (i=0; i< 5; i++) {
      printf("%f (%f) ",  get_value_double(key_index, my_sample + i*row_size ), get_value_double(key_index+1, my_sample + i*row_size ));
    }
    printf("\n");
  }
...
int phase2(int mpi_rank, int mpi_size, char *data, int my_data_size, char *pivots, int rest_size, int row_size){
  ...
  int ii, jj, replicated_jj, replicated_jj_start, previous_ii;
  ...
  unsigned long long     dest_rank, cur_rank;
  if(mpi_rank != (mpi_size -1)){
  ...
  for(k = 0; k < mpi_size; k++){
    if(dest != (mpi_size -1)){
      ...
      if(rank_combine == 1)
        dest_rank    = getULLValue(key_index+1, pivots + dest*row_size);
    }else{
    ...
    if(skew_data == 1){ 
      ...
      if(dest_pivot == my_pivot && mpi_rank ==  dest){
        ...
        if(dest_pivot_replicated == 0){
          ...
          //if(mpi_rank == 0)
          //  printf("rank less %d , rank qual %d, partition_size %d, (my rank %d, dest = %d,  p_value_head %f, dest_pivot %f ) \n", rank_less, rank_equal, partition_size, mpi_rank, dest, p_value_head, dest_pivot);
      ...
      for (ii=previous_ii; ii < my_data_size; ii++){
        ...
        if(dest_pivot_replicated == 1){
          cur_rank  = getULLValue(key_index+1, data+ii*row_size);
          if( cur_value <= dest_pivot && cur_rank <= dest_rank){
            //memcpy(send_buff+jj*row_size, data+ii*row_size, row_size);
            jj++;
          }else{
            break;
  ...
  for (i = 0; i < mpi_size; i++){
    ...
    ssize = ssize + scount[i];
  }
  
  if(ssize != my_data_size){
    printf("Size mismatch after assigning on pivots: send size (%d), my_data_size (%lld) ! \n", ssize, my_data_size);
    exit(-1);
  }
  ...
  if(rank_combine == 0){
    ...
    if(final_buff == NULL){
      printf("Allocation for final_buff fails (rank %d, size %d  !\n", mpi_rank, rsize);
      exit(0);
    }
  ...
  double load, sum_load, sq_load, sumsq_load, var_load, max_load, min_load;
  load = rsize;
  MPI_Allreduce(&load, &max_load, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  MPI_Allreduce(&load, &min_load, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
  MPI_Allreduce(&load, &sum_load, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  sq_load = load - (sum_load / mpi_size);
  sq_load = sq_load * sq_load;
  MPI_Allreduce(&sq_load, &sumsq_load, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  if(mpi_size > 1) {
  ...
  if(mpi_rank == 0)
    printf("Exchange data ends, my_rank %d, orgi_size %d, new_size %d, taking [%f]s, (max %f, min %f, var %f) \n", mpi_rank, my_data_size, rsize, (t2-t1), max_load, min_load, var_load);
...
int master(int mpi_rank, int mpi_size, char *data, int my_data_size, int rest_size, int row_size){
  ...
  for(i = 0; i < (mpi_size-1); i++){
    ...
    cur_rank  = getULLValue(key_index+1, pivots+row_size*i);
    //if(previous_pivot ==  cur_pivot && i < 20){
    //  printf(" (S)");
    //}
    ...
    //if(i < 200)
    //  printf("%f(r-%llu),", cur_pivot, cur_rank);
  }
  printf("\n");
  free(all_samp);
  ...
  j = 0;
  double next_pivot;
  for(i=0; i < (mpi_size -2); i++){
    cur_pivot = get_value_double(key_index, pivots + i*row_size);
    //next_pivot = get_value_double(key_index, pivots + (i+1)*row_size);
    //if(curr_pivot == next_pivot){
    for( j = i+1; j < (mpi_size -1); j++){
      next_pivot=get_value_double(key_index, pivots + j*row_size);
      if(cur_pivot == next_pivot){
        continue;
      }else{
        break;
      }
    ...
    if( (i+1) < j){
      //reorder_data_on_rank(pivots, i,  j);
      qsort(pivots+i*row_size, j-i, row_size, CompareULLKeyRank);
    }
  ...
  for(i=0; i< (mpi_size-1) ; i++){
    cur_pivot = get_value_double(key_index, pivots + i*row_size);
    cur_rank = getULLValue(key_index+1, pivots + i*row_size);
    printf("%f(r-%llu) ", cur_pivot, cur_rank);
    if(i > 10)
      break;
  }
...
unsigned long long getULLValue(int index, char *row_data){
  ...
  p_offset = max_type_size * index;
  p = (unsigned long long *)(row_data + p_offset);
  value = *p;
